...
class NoFAM(nn.Module):
    """
    不带注意力的简易 FAM 替代：
      - 直接在深度/时间维 cat(x1, x2)
      - 用 conv(kernel_size=1) 保持输出通道不变
      - 不再进行时间池化和全连接的注意力操作
    """
    def __init__(self, in_channels):
        super(NoFAM, self).__init__()
        # 与原 FAM 相同的卷积输出通道数，以保持后续网络逻辑一致
    ...
    def forward(self, x1, x2):
        """
        x1, x2: 形状均为 [N, in_channels, T, H, W]
        最终输出: [N, in_channels, T', H, W], 其中 T' = T1 + T2
        """
        # 1) 不做任何注意力操作，直接用 x1、x2
        # 2) 在深度维 (dim=2) 拼接 => 形状 (N, in_channels, T1+T2, H, W)
        x_fam = torch.cat([x1, x2], dim=2)

        # 3) 通过 1×1卷积 => 形状仍是 (N, in_channels, T1+T2, H, W)
        x_fam = self.conv(x_fam)
        return x_fam
...
class MultiModalModel_woFAM(nn.Module):
    def __init__(self):
        super(MultiModalModel_woFAM, self).__init__()
        ...
        # ------------ 用 NoFAM 替代原 FAM ------------
        self.fam_fused_1 = NoFAM(in_channels=128)
        self.fam_fused_2 = NoFAM(in_channels=128)
        self.fam_eeg_1   = NoFAM(in_channels=128)
        self.fam_eeg_2   = NoFAM(in_channels=128)
    ...
    def forward(self, x_fnirs, x_eeg):
        ...
        # 2) Gated Fusion
        fused = self.gated_fusion(x_fnirs, x_eeg)   # => (N,64,30,16,16) (与Baseline保持一致)

        # 3) CBR
        x_fnirs = self.cbr_fnirs_1(x_fnirs)         # => (N,64,30,16,16)
        x_eeg   = self.cbr_eeg_1(x_eeg)             # => (N,128,360,16,16)

        fused   = torch.cat([fused, x_fnirs], dim=1)# => (N,64+64=128,30,16,16)
        x_fnirs = self.cbr_fnirs_2(x_fnirs)         # => (N,128,30,16,16)

        # 4) NoFAM (替换 FAM)
        fused   = self.fam_fused_1(x_fnirs, fused)  # => (N,128, 30+30=60, 16,16) => conv => (N,128,60,16,16)
        x_eeg   = self.fam_eeg_1(x_fnirs, x_eeg)    # => (N,128, ???,16,16), 与Baseline同形状

        fused   = self.cbr_fused_1(fused)           # => (N,128,60,16,16)
        x_eeg   = self.cbr_eeg_2(x_eeg)             # => (N,128,???)

        x_fnirs = self.cbr_fnirs_3(x_fnirs)         # => (N,256,30,16,16)
        x_fnirs = self.cbr_fnirs_4(x_fnirs)         # => (N,128,30,16,16)

        fused   = self.fam_fused_2(x_fnirs, fused)  # => (N,128, ???,16,16)
        x_eeg   = self.fam_eeg_2(x_fnirs, x_eeg)    # => (N,128, ???,16,16)
        fused   = self.cbr_fused_2(fused)           # => (N,128, ???,16,16)

        torch.cuda.empty_cache()

        # 5) STA
        x_eeg   = self.cbr_eeg_3(x_eeg)             # => (N,128, ???,16,16)
        x_fnirs = self.sta_fnirs(x_fnirs)           # => (N,128,30,16,16)
        x_eeg   = self.sta_eeg(x_eeg)               # => (N,128, ???,16,16)
        fused   = self.sta_fused(fused)             # => (N,128, ???,16,16)
